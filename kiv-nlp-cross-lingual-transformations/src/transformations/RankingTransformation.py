...
class RankingTransformation(Transform):
    """
        Implementation of Ranking Transformation
    ...
    def __init__(self,
        ...
        logger.info("RankingTransformation initialized")
    ...
    def compute_T(self, X_src, Y_trg):
        ...
        for iteration in range(iterations):
            for batch_start in range(0, X.size()[0], batch_size):
                ...
                distances = torch.cdist(Y_pred, Y_batch) - torch.cdist(Y_batch, Y_batch)
                distances.fill_diagonal_(float('inf'))  # i != j
                Y_negatives = Y_batch[torch.squeeze(distances.topk(negative_examples, largest=False).indices)]

                # Compute loss value using hinge loss function
                # Sum over all negative examples
                negatives = delta + torch.linalg.norm(Y_pred - Y_batch, dim=1)[:, None] - torch.linalg.norm(
                    Y_pred[:, None] - Y_negatives, dim=2)
                batch_loss = torch.sum(torch.maximum(torch.tensor(0.0, dtype=torch.float32), negatives))
            ...
            if self.verbose is True:
                logger.info(70 * "-")
                logger.info(f"Iteration: {iteration}\tSum Total Loss: {total_loss:.5f}\t"
                            f"Cosine similarity: {np.mean(cosine):.4f}")
                logger.info(70 * "-")
    ...
    def __str__(self):
        return "RankingTransformation"


if __name__ == '__main__':
    from tests.RankingTransformationTest import mainRTtest
